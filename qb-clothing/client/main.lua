            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Open Clothing Shop')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - Open Barber Shop')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Barber", selected = true},
                                    })
                                end
                            end
                        end
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                if dist < 15 then
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if ESX.PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~g~E~w~ - Open Outfits Menu')
                                if IsControlJustPressed(0, Keys["E"]) then
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('qb-clothing:delOutfit')
